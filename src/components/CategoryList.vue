  ...
  <div class="mx-auto max-w-5xl mt-4 md:mt-6">
    <h4 class="flex items-center justify-between mb-3">
      <div class="flex items-center">
        <svg
          class="text-indigo-600 dark:text-indigo-500 inline-block"
      ...
      <div
        v-if="(categories.length > 3 && isMobile) || (categories.length > 6 && !isMobile)"
        class=""
      >
        <p
          v-if="showAllCategories"
          @click="showAllCategories = false"
          class="cursor-pointer text-indigo-600 dark:text-indigo-500 flex items-center"
        >
          Hide
          <svg
            class="inline-block"
    ...
    <div class="grid grid-cols-3 md:grid-cols-6 gap-3">
      <div
        ...
        :key="index"
        class="group animate__animated animate__bounceIn cursor-pointer bg-white dark:bg-zinc-800 h-fit hover:bg-slate-50 dark:hover:bg-zinc-700 duration-150 shadow-sm hover:shadow px-3 py-2 group rounded"
      >
        <img
          :src="c.image"
          alt="category img"
        ...
        <p
          class="mt-2 text-sm md:text-base group-hover:text-blue-500 text-center text-zinc-800 dark:text-slate-200 font-medium"
        >
          {{ c.name }}
        </p>
        <p class="text-center text-sm text-gray-500 dark:text-muted font-semibold">
...
export default {
  ...
  methods: {
    ...
    getCategoryList() {
      ...
      axios
        .get(`${import.meta.env.VITE_API_BASE_URL}/api/category/listWithCount`, {
          headers: {
        ...
        .then((response) => {
          for (let i = 0; i < response.data.categories.length; i++) {
            if (response.data.categories[i].image) {
              response.data.categories[i].image =
                `${import.meta.env.VITE_API_BASE_URL}/storage/categoryImages/` +
                response.data.categories[i].image;
            } else {
              response.data.categories[i].image =
                `${import.meta.env.VITE_API_BASE_URL}/images/default.png`;
            }
          }
