<template>
    ...
    <div  v-for="(quiz,index) in quizzes.data" :key="index" class="pb-2 animate__animated animate__bounceIn group bg-white dark:bg-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-700 shadow rounded border-[1.5px] border-slate-200 dark:border-zinc-500">
...
export default {
    ...
    methods: {
          ...
          axios.post(`http://127.0.0.1:8000/api/quiz/saveQuiz`,{
                    ...
                    },
                    {
                      ...
                      if(response.data.status === 'saved'){
                        this.saved_quizzes.push({'quiz_id':quiz_id});
                      }else if(response.data.status === 'unsaved'){
                        const index = this.saved_quizzes.findIndex(q=>q.quiz_id==quiz_id);
                        this.saved_quizzes.splice(index,1);
                      }
        ...
        isQuizSaved(quiz_id) {
          return this.saved_quizzes.some(savedQuiz => savedQuiz.quiz_id === quiz_id);
        },
        ...
        getSavedQuizzes(){
            this.setLoadingStatus(true);
            axios.get('http://127.0.0.1:8000/api/quiz/getSavedQuizzes',{
                        headers : {
                            'Authorization' : `Bearer ${this.getToken}`,
                        }
                    })
            .then((response) => {
                if(response.data.status == true){
                    this.setLoadingStatus(false);
                    this.saved_quizzes = response.data.saved_quizzes;
                }
            })
            .catch(error => console.log(error));
        },
