    ...
    <div class="flex md:mt-6 flex-col items-center justify-center animate__animated animate__fadeIn">
        <div class="w-full mt-4 p-6 md:w-2/4 bg-white dark:bg-zinc-800 shadow-sm rounded">
            ...
            <form @submit.prevent="changePassword" class="px-3 py-2">
                ...
                <div v-if="isOldPassword" class="mb-5">
                    <div class="relative z-0 w-full group input-gp">
                    <input required name="oldPassword" v-model="passwords.oldPassword" :type="showPassword.oldPassword?'text':'password'" class=" dark:text-white px-3 relative block py-3 w-full text-sm text-gray-900 bg-transparent appearance-none border border-[1.5px] border-slate-300 dark:border-zinc-700 rounded-md dark:focus:border-blue-500 -z-0 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                    ...
                    <span v-if="formStatus === false && message.oldPassword" class="text-red-600 text-sm ms-1">{{ message.oldPassword[0] }}</span>
                    
                </div>
                <div class="mb-5">
                    <div class="relative z-0 w-full group input-gp">
                    <input required name="newPassword" v-model="passwords.newPassword" :type="showPassword.newPassword?'text':'password'" class=" dark:text-white px-3 relative block py-3 w-full text-sm text-gray-900 bg-transparent appearance-none border border-[1.5px] border-slate-300 dark:border-zinc-700 rounded-md dark:focus:border-blue-500 -z-0 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                    ...
                    <span v-if="formStatus === false && message.newPassword" class="text-red-600 text-sm ms-1">{{ message.newPassword[0] }}</span>

                    
                </div>
                <div class="mb-5">
                    <div class="relative z-0 w-full group input-gp">
                    <input required name="confirmPassword" v-model="passwords.confirmPassword" :type="showPassword.confirmPassword?'text':'password'" class=" dark:text-white px-3 relative block py-3 w-full text-sm text-gray-900 bg-transparent appearance-none border border-[1.5px] border-slate-300 dark:border-zinc-700 rounded-md dark:focus:border-blue-500 -z-0 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
                    ...
                    <span v-if="formStatus === false && message.confirmPassword" class="text-red-600 text-sm ms-1">{{ message.confirmPassword[0] }}</span>

                </div>
                <a v-if="isOldPassword" @click="directForgotPassword" class="text-blue-500 hover:text-blue-600 duration-100">Forgotten your password?</a>
                <button type="submit" class="font-medium flex items-center mt-2 float-right text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300  rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-zinc-700 dark:hover:bg-zinc-600 focus:outline-none dark:focus:ring-blue-800">Change Password</button>
    ...
    methods: {
      ...
      checkOldPassword(){
        ...
        axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/account/isOldPassword`,null,{
                ...
                this.isOldPassword = response.data.isOldPassword;
                this.setLoadingStatus(false);
        }).catch(error => console.log(error)); 
      }
