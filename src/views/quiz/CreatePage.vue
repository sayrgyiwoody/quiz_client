        ...
        <div class="md:col-span-3 md:px-6 md:py-3">
                    ...
                    <form @submit.prevent="saveQuizInfo">
                        <div action="" class="">
                            <div class="relative z-0 w-full mb-4 group">
                                <input @input="titleFocused=true" @blur="titleFocused=false" v-model="quiz_title" :maxlength="maxTitle" name="" type="text" class=" dark:text-white px-3 relative block py-3 w-full text-sm text-gray-900 bg-transparent appearance-none border-[1.5px] border-slate-300 dark:border-zinc-700 rounded-md dark:focus:border-blue-500 -z-0 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                                ...
                                <span v-if="titleFocused" class=" text-primary dark:text-blue-500 px-1 text-sm">{{ remainingTitle }} characters for title remaining</span>
            ...
            <div class="mt-3 bg-white dark:bg-zinc-800 px-4 py-3 rounded shadow animate__animated animate__bounceIn">
                <div class="flex justify-between">
                    ...
                    <div v-if="questionMsg" class="flex items-center border-[1.5px] dark:border-zinc-700 text-indigo-500 font-semibold rounded px-4 py-3 mb-4">
                        <svg class=" inline-block me-2" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M12.3 7.29c.2-.18.44-.29.7-.29c.27 0 .5.11.71.29c.19.21.29.45.29.71c0 .27-.1.5-.29.71c-.21.19-.44.29-.71.29c-.26 0-.5-.1-.7-.29c-.19-.21-.3-.44-.3-.71c0-.26.11-.5.3-.71m-2.5 4.68s2.17-1.72 2.96-1.79c.74-.06.59.79.52 1.23l-.01.06c-.14.53-.31 1.17-.48 1.78c-.38 1.39-.75 2.75-.66 3c.1.34.72-.09 1.17-.39c.06-.04.11-.08.16-.11c0 0 .08-.08.16.03c.02.03.04.06.06.08c.09.14.14.19.02.27l-.04.02c-.22.15-1.16.81-1.54 1.05c-.41.27-1.98 1.17-1.74-.58c.21-1.23.49-2.29.71-3.12c.41-1.5.59-2.18-.33-1.59c-.37.22-.59.36-.72.45c-.11.08-.12.08-.19-.05l-.03-.06l-.05-.08c-.07-.1-.07-.11.03-.2M22 12c0 5.5-4.5 10-10 10S2 17.5 2 12S6.5 2 12 2s10 4.5 10 10m-2 0c0-4.42-3.58-8-8-8s-8 3.58-8 8s3.58 8 8 8s8-3.58 8-8"/></svg>
                        {{ questionMsg }}
                    </div>
                    ...
                    <form v-if="option" @submit.prevent="addQuestion">
                        ...
                        <a @click="cancelEdit()" v-if="editStatus" class="me-4 inline-block items-center justify-center text-white dark:text-slate-50 px-4 py-2 bg-primary hover:bg-primary_hover rounded dark:bg-zinc-700 dark:hover:bg-zinc-600 duration-150">
                        ...
                        <button v-else class=" flex items-center justify-center text-white dark:text-slate-50 px-4 py-2 bg-primary hover:bg-primary_hover rounded dark:bg-zinc-700 dark:hover:bg-zinc-600 duration-150">
                            ...
                            <span class="ms-2 font-medium">Add to List</span>
                        </button>
        ...
        <div class=" col-span-4">
            <div class=" md:px-6 md:py-3 mt-4 ">
                ...
                <div v-if="questionList.length > 0" class="flex items-center mt-6 ">
                    <button v-if="!isLoading" @click="publish" type="button" class="animate__animated animate__bounceIn me-4 group flex items-center py-3 px-5 mr-2 mb-2 text-sm font-medium text-zinc-900 focus:outline-none bg-white rounded-lg border border-zinc-200 hover:bg-slate-50 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-zinc-700 dark:bg-zinc-800 dark:text-slate-400 dark:border-zinc-600 dark:hover:text-white dark:hover:bg-zinc-700">
                    ...
                    <button v-else disabled type="button" class="animate__animated animate__bounceIn me-4 group flex items-center py-3 px-5 mr-2 mb-2 text-sm font-medium text-zinc-900 focus:outline-none bg-white rounded-lg border border-zinc-200 hover:bg-slate-50 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-zinc-700 dark:bg-zinc-800 dark:text-slate-400 dark:border-zinc-600 dark:hover:text-white dark:hover:bg-zinc-700">
                        <svg aria-hidden="true" class="w-8 h-8 me-3 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/></svg>
...
export default {
    ...
    methods: {
        ...
        addQuestion(){
            if(!this.answer){
                this.questionMsg = "Please fill answer";
                setTimeout(() => {
            ...
            if(this.option === 'choice' && this.answer){
                ...
                if(answerExist === false){
                    this.questionMsg = "Please select answer for question";
                    setTimeout(() => {
                    this.questionMsg = null;
                    }, 2000); 
                    return;
