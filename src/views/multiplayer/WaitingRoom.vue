<template>
    <div class="max-w-lg mx-auto mt-4 md:mt-10 font-poppins mb-4">
        <button  class="flex items-center group  text-zinc-700 hover:text-zinc-800 dark:text-slate-200 dark:hover:text-white  py-2 rounded mt-3 font-medium">
    ...
    <div class="relative font-inter max-w-lg mx-auto bg-white dark:bg-zinc-800 px-5 pt-3 pb-14 rounded-xl  shadow">
        <p class="flex items-center space-x-2 mb-2">
            ...
            <span class=" font-semibold text-xl text-zinc-800 dark:text-slate-100">Share room code</span>
        </p>
        ...
        <div class="absolute -bottom-8 w-full flex justify-center left-0">
    ...
    <div class="max-w-5xl min-h-[200px] pt-12 px-8 flex flex-col items-center justify-center mx-auto bg-white dark:bg-zinc-800 pb-4 mt-16 md:mt-20 rounded relative font-inter">
        ...
        <div class="flex flex-wrap justify-center items-center">
            <img v-for="(participant,index) in participants" :key="index" class="mb-3 animate__animated animate__bounceIn rounded-full w-16 h-16 me-2 object-cover" referrerpolicy="no-referrer" :src="getImageUrl(participant)" alt="profile image">
        </div>
        

        <div v-if="participants.length === 0" class="waiting-loader mb-5 dark:sr-only">
            <span class="bar bar-light"></span>
            <span class="bar bar-light"></span>
            <span class="bar bar-light"></span>
        </div>
        <div v-if="participants.length === 0" class="waiting-loader dark:mb-5 sr-only dark:not-sr-only">
            <span class="bar bar-dark"></span>
            <span class="bar bar-dark"></span>
            <span class="bar bar-dark"></span>
        </div>
        
        <p v-if="participants.length === 0" class="text-center text-slate-600 dark:text-muted">No participants have joined, Ask them to join using the room code above.</p>
    </div>
</template>
...
export default {
    ...
    mounted () {
      window.Echo.channel(`room.${this.$route.params.room_code}`)
