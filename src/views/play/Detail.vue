<template>
  <div class="max-w-xl mx-auto mt-3 md:mt-6 mb-4 font-poppins">
    <button
      @click="returnBack()"
      class="flex items-center group text-zinc-700 hover:text-zinc-800 dark:text-slate-200 dark:hover:text-white px-4 py-2 rounded mt-3 font-medium"
    >
      <svg
        class="me-2 group-hover:-translate-x-1 duration-150 inline-block"
  ...
  <div
    ...
    class="bg-white dark:bg-zinc-800 px-6 py-4 rounded shadow max-w-xl mx-auto font-poppins"
  >
    <h4
      class="capitalize rounded-l-sm border-l-4 ps-2 border-rose-500 dark:border-rose-500 text-zinc-900 dark:text-white text-2xl font-semibold mb-3"
    >
      {{ quiz.title }}
    </h4>
    <div class="mb-3 flex items-center">
      <img
        class="rounded-full w-16 h-16 me-2 object-cover"
      ...
      <div class="flex flex-col">
        <div
          class="text-zinc-800 dark:text-slate-100 font-medium flex items-center flex-wrap"
        >
          <span class="font-semibold text-lg">{{
            quiz.user_name ? quiz.user_name : "Unknown"
          }}</span>
        ...
        <p class="text-gray-600 dark:text-muted font-medium text-sm">
    ...
    <div class="flex space-x-2">
      <div
        class="text-xs cursor-pointer text-zinc-800 duration-150 mb-3 px-3 border-[1.5px] border-slate-200 dark:border-zinc-600 dark:text-slate-200 w-fit py-2 rounded-2xl flex items-center"
      >
        <svg
          class="me-2 inline-block w-5 h-5"
          xmlns="http://www.w3.org/2000/svg"
          width="32"
          height="32"
          viewBox="0 0 24 24"
      ...
      >
        <svg
          class="me-2 inline-block w-4 h-4"
          xmlns="http://www.w3.org/2000/svg"
          width="32"
          height="32"
          viewBox="0 0 36 36"
    ...
    <div
      class="font-inter text-sm bg-slate-50 border border-slate-200 dark:border-zinc-600 dark:bg-zinc-700 px-4 py-3 mb-2 rounded text-zinc-600 dark:text-slate-200"
    >
      {{ quiz.desc }}
    </div>
    <div class="flex items-center space-x-2">
      ...
      <button
        ...
        class="px-6 flex items-center group bg-indigo-600 hover:bg-indigo-500 dark:bg-indigo-500 hover dark:hover:bg-indigo-600 text-white py-2 rounded-md mt-3 font-medium"
      >
        Play
        <svg
          class="ms-2 group-hover:translate-x-1 duration-150 inline-block w-6 h-6 mt-1"
          xmlns="http://www.w3.org/2000/svg"
          width="32"
          height="32"
          viewBox="0 0 256 256"
  ...
  >
    ...
    >
      <div class="animate-pulse">
        <div class="h-2.5 bg-slate-300 rounded-full dark:bg-zinc-600 w-32 mb-2"></div>
        <div class="w-48 h-2 bg-slate-300 rounded-full dark:bg-zinc-600"></div>
      </div>
    </h4>
    <div class="mb-3 flex items-center">
      <div class="flex items-center my-2">
        <svg
          class="animate-pulse rounded-full w-16 h-16 me-3 text-slate-300 dark:text-zinc-600"
          aria-hidden="true"
          xmlns="http://www.w3.org/2000/svg"
          fill="currentColor"
          viewBox="0 0 20 20"
    ...
    <div class="animate-pulse">
      <div class="h-2 bg-slate-300 rounded-full dark:bg-zinc-600 mb-2.5"></div>
      <div class="h-2 bg-slate-300 rounded-full dark:bg-zinc-600 mb-2.5"></div>
      <div class="h-2 bg-slate-300 rounded-full dark:bg-zinc-600 mb-2.5"></div>
      <div class="h-2 bg-slate-300 rounded-full dark:bg-zinc-600 mb-2.5"></div>
      <div class="h-2 bg-slate-300 rounded-full dark:bg-zinc-600 mb-3"></div>
    </div>
    <div class="flex items-center space-x-2">
      <!-- <button @click="createRoom" class=" font-inter border-[1.5px] border-sky-500 dark:border-sky-500 flex items-center group hover:bg-sky-500 duration-200 dark:hover:bg-sky-500 hover:text-white dark:hover:text-slate-50  text-sky-500 dark:text-sky-500 px-3 py-2 rounded-lg mt-3 font-medium ">
                Multiplayer
...
export default {
  name: "Detail",
  computed: {
    ...mapGetters(["getToken", "getLoadingStatus"]),
  },
  data() {
    return {
      quiz: {},
  ...
  methods: {
    ...
    getQuiz() {
      ...
      axios
        .post(
          `${import.meta.env.VITE_API_BASE_URL}/api/quiz/detail`,
          {
        ...
        .then((response) => {
          ...
          if (
            response.data.quiz.provider_avatar != null &&
            response.data.quiz.user_image === null
          ) {
    ...
    generateRoomCode() {
      ...
      axios
        .post(
          `${import.meta.env.VITE_API_BASE_URL}/api/multiplayer/generateRoom`,
          {
            quiz_id: this.$route.params.quiz_id,
          },
          {
