<template>
    <div class=" max-w-lg mx-auto px-2 py-3">
        ...
        <div class="flex space-x-2">
            <div v-for="(question,index) in questionList" :key="index" :class="{'bg-emerald-600':answerHistory[index]===true,'bg-rose-600':answerHistory[index]===false}" class=" w-full h-3  border dark:border-none border-slate-300 shadow rounded-[3px] cursor-pointer"></div>
        ...
        <div v-for="(question,index) in paginatedQuestions" :key="index" class=" mt-6 animate__animated animate__bounceIn">
            ...
            <button :disabled="answerStatus" @click="checkAnswer(quiz.quiz_id,question.id)" type="button" class=" disabled:bg-blue-700 dark:disabled:bg-zinc-700 cursor-pointer mt-4 animate__animated animate__bounceIn flex items-center justify-center text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-md  px-5 py-2.5 text-center w-full dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
